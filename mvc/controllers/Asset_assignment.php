...
class Asset_assignment extends Admin_Controller {
    ...
    public function unique_assetID() {
        if($this->input->post('assetID') == 0) {
            $this->form_validation->set_message('unique_assetID', 'El campo %s es obligatorio.');
            return FALSE;
        }
        return TRUE;
    }
        ...
        if($quantity && $assetID) {
            ...
            if(count($asset_purchases)) {
                ...
                if ($quantity <= $balance_quantity) {
                    ...
                    $this->form_validation->set_message("valid_quantity", "El artículo no está disponible en esta cantidad");
                    return FALSE;
                }

            } else {
                $balance_quantity = $total_quantity;
                $this->form_validation->set_message("valid_quantity", "Por favor, primero compre");
                return FALSE;
            }
        }
        return TRUE;
