...
Class Issue extends Admin_Controller {
	...
	public function unique_quantity() {
		...
		if($id) {
			if((int)$bookID) {
				...
				$this->form_validation->set_message("unique_quantity", "%s no están disponibles..");
				return FALSE;
			}
		} else {
			if((int)$bookID) {
	...
	public function unique_lID() {
		...
		if(!count($lID)) {
			$this->form_validation->set_message("unique_lID", "%s está mal.");
			return FALSE;	
		}
		return TRUE;
	}
	...
	public function unique_book() {
		...
		if($id) {
			...
			if(count($book)) {
				...
				$this->form_validation->set_message("unique_book", "%s ya prestado.");
				return FALSE;
			}
		} else {
	...
	public function unique_book_call() {
		if($this->input->post('book') === '0') {
			$this->form_validation->set_message("unique_book_call", "El campo %s es obligatorio.");
	     	return FALSE;
		}
		return TRUE;
	}
		...
		if((int)$bookID && $bookID != "") {
			...
			if($bookandauthor) {
				if($bookandauthor->author == $author) {
					...
					$this->form_validation->set_message("match_bookauthor", "%s dosis de autor no coincide.");
					return FALSE;
				}
			} else {
				$this->form_validation->set_message("match_bookauthor", "%s dosis de autor no coincide.");
	...
	public function valid_number () {
		if($this->input->post('fine') && $this->input->post('fine') < 0) {
			$this->form_validation->set_message("valid_number", "%s no es un número válido");
			return FALSE;
		}
		return TRUE;
	}
