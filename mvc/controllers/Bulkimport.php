...
class Bulkimport extends Admin_Controller {
                        ...
                        foreach ($csv_array as $row) {
                            ...
                            if (count($match) <= 0) {
                                ...
                                if($singleteacherCheck['status']) {
                                    ...
                                    $msg .= $i.". ". $row['Name']." no se agrega! , ";
                                    $msg .= implode(' , ', $singleteacherCheck['error']);
                                    $msg .= ". <br/>";
                                }
                            } else {
                                $this->session->set_flashdata('error', "Archivo csv incorrecto!");
                                redirect(base_url("bulkimport/index"));
                            }
                            $i++;
                        }
    ...
    private function singleteacherCheck($array) {
        ...
        if($name && $designation && $dob && $gender && $religion && $email && $phone && $address && $jod && $username && $password) {
            ...
            if(!$name) {
                $retArray['error']['name'] = 'Nombre de docente inválido';
            }
            if(!$designation) {
                $retArray['error']['designation'] = 'Designación inválida';
            }
            if(!$dob) {
                $retArray['error']['dob'] = 'Fecha de nacimiento invalida';
            }
            if(!$gender) {
                $retArray['error']['gender'] = 'Genero inválido';
            }
            if(!$religion) {
                $retArray['error']['religion'] = 'Religión inválido';
            }
            if(!$email) {
                $retArray['error']['email'] = 'La dirección de correo electrónico o la dirección de correo electrónico no válidas ya existen.';
            }
            if(!$phone) {
                $retArray['error']['phone'] = 'Número de telefono inválido';
            }
            if(!$address) {
                $retArray['error']['address'] = 'Dirección inválida';
            }
            if(!$jod) {
                $retArray['error']['jod'] = 'Fecha de nacimiento invalida';
            }
            if(!$username) {
                $retArray['error']['username'] = 'Nombre de usuario o nombre de usuario no válido ya existe';
            }
            if(!$password) {
                $retArray['error']['password'] = 'Contraseña inválida';
            }
        }
        return $retArray;
    }
    ...
    private function singleparentCheck($array) {
        ...
        if($name && $father_name && $mother_name && $father_profession && $mother_profession && $email && $phone && $address && $username && $password) {
            ...
            $retArray['status'] = FALSE;
            if(!$name) {
                $retArray['error']['name'] = 'Nombre de padre inválido';
            }
            if(!$father_name) {
                $retArray['error']['father_name'] = 'Nombre de padre inválido';
            }
            if(!$mother_name) {
                $retArray['error']['mother_name'] = 'Nombre de la madre no válido';
            }
            if(!$father_profession) {
                $retArray['error']['father_profession'] = 'Profesión de padre inválido';
            }
            if(!$mother_profession) {
                $retArray['error']['mother_profession'] = 'Profesión de madre inválida';
            }
            if(!$email) {
                $retArray['error']['email'] = 'La dirección de correo electrónico o la dirección de correo electrónico no válidas ya existen.';
            }
    ...
    private function singleuserCheck($array) {
        ...
        if($name && $dob && $gender && $religion && $email && $phone && $address && $jod && $username && $password && $usertype) {
            ...
            $retArray['status'] = FALSE;
            if(!$name) {
                $retArray['error']['name'] = 'Nombre de usuario no válido';
            }
            if(!$dob) {
                $retArray['error']['dob'] = 'Fecha de nacimiento invalida';
            }
            if(!$gender) {
                $retArray['error']['gender'] = 'Género inválido';
            }
            if(!$religion) {
                $retArray['error']['religion'] = 'Religión inválido';
            }
            ...
            if(!$usertype) {
                $retArray['error']['usertype'] = 'Tipo de usuario no válido';
            }
        }
        return $retArray;
    }
    ...
    private function singlebookCheck($array) {
        ...
        if($book && $price && $rack && $author && $quantity && $subject_code) {
            ...
            if(count($books)) {
                $retArray['status'] = FALSE;
                $retArray['error']['book'] = 'Libro ya existe';
            } else {
                $retArray['status'] = TRUE;
            }
        } else {
            $retArray['status'] = FALSE;
            if(!$book) {
                $retArray['error']['book'] = 'Nombre de libro inválido';
            }
            if(!$price) {
                $retArray['error']['price'] = 'Precio no son validos';
            }
            if(!$rack) {
                $retArray['error']['rack'] = 'El Rack no es valido';
            }
            if(!$author) {
                $retArray['error']['author'] = 'Autor no son validos';
            }
            if(!$quantity) {
                $retArray['error']['quantity'] = 'La cantidad no es válida';
            }
            if(!$subject_code) {
                $retArray['error']['subject_code'] = 'Código del sujeto no son válidos';
            }
        }
        return $retArray;
    }
    ...
    public function singlestudentCheck($array) {
        ...
        if($name && $dob && $gender && $religion && $email && $phone && $address && $class && $section && $username && $password && $roll && $bloodgroup && $state && $country && $registrationno && $group && $optionalsubject && $checkStudent) {
            ...
            if(!$class) {
                $retArray['error']['class'] = 'Clase inválida';
            }
            if(!$section) {
                $retArray['error']['section'] = 'Sección inválida';
            }
            if(!$username) {
                $retArray['error']['username'] = 'Nombre de usuario o nombre de usuario no válido ya existe';
            }
            ...
            if(!$roll) {
                $retArray['error']['roll'] = 'El rollo o rollo no válido ya existe en la clase';
            }
            if(!$bloodgroup) {
                $retArray['error']['bloodgroup'] = 'Grupo sanguíneo no válido';
            }
            if(!$state) {
                $retArray['error']['state'] = 'Estado Inválido';
            }
            if(!$country) {
                $retArray['error']['country'] = 'País inválido';
            }
            if(!$registrationno) {
                $retArray['error']['registrationno'] = 'El registro no es válido o el registro ya no existe';
            }
            if(!$group) {
                $retArray['error']['group'] = 'Grupo inválido';
            }
            if(!$optionalsubject) {
                $retArray['error']['optionalsubject'] = 'Asunto opcional no válido';
            } 
            if(!$checkStudent) {
                $retArray['error']['checkStudent'] = 'El estudiante no puede ser agregado en la sección';
            }
        }
        return $retArray;
    }
