...
class Resetpassword extends Admin_Controller {
	...
	function __construct() {
		...
		if(config_item('demo')) {
			$this->session->set_flashdata('error', 'El módulo de Restablecer contraseña del DEMO está deshabilitado');
	...
	public function unique_users() {
		if($this->input->post('users') == 0) {
			$this->form_validation->set_message("unique_users", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
	...
	public function unique_username() {
		if($this->input->post('username') == 0) {
			$this->form_validation->set_message("unique_username", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
