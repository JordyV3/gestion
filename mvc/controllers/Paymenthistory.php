...
class Paymenthistory extends Admin_Controller {
	...
	public function valid_number() {
		if($this->input->post('amount') != 0) {
			...
			$this->form_validation->set_message("valid_number", "Dame una cantidad valida no cero");
			return FALSE;
		}
		return TRUE;
	}
		...
		if(count($this->data['payment'])) {
			...
			if(count($this->data['invoice'])) {
				...
				if($this->input->post('amount') > $this->data['dueamount']) {
					$this->form_validation->set_message("unique_amount", "El %s es mayor que la cantidad debida");
					return FALSE;
				}
				return TRUE;
			} else {
	...
	public function unique_paymentmethod() {
		if($this->input->post('payment_method') === '0') {
			$this->form_validation->set_message("unique_paymentmethod", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
