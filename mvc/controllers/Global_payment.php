...
class Global_payment extends Admin_Controller
{
    ...
    public function unique_student() {
        if($this->input->post('studentID') == 0) {
            $this->form_validation->set_message("unique_student", "El campo %s es obligatorio");
            return false;
        }
        return true;
    }
    ...
    public function unique_paidweaverfine() {
                ...
                if($paid['value'] != '') {
                    if((float) $paid['value']) {
                        if(strlen($paid['value']) <= $max_value && strlen($paid['value']) >= 0) {
                            ...
                            $this->form_validation->set_message("unique_paidweaverfine", "%s no puede exceder ".$max_value." caracteres de longitud.");
                            return FALSE;
                        } 
                    } else {
                        $this->form_validation->set_message("unique_paidweaverfine", "%s debe contener solo números.");
                        return FALSE;
                    }
                }
            }
        ...
        if($paidRequiredStatus || $weaverRequiredStatus || $fineRequiredStatus) {
            if($this->session->flashdata('paymentGenerateStatus')) {
                return FALSE;
                $this->form_validation->set_message("unique_paidweaverfine", "Se requiere %s.");
            }
