...
class Holiday extends Admin_Controller {
	...
	public function todate_valid($date) {
		...
		if(strlen($date) < 10) {
			$this->form_validation->set_message("todate_valid", "%s no es válido dd-mm-aaaa");
				return FALSE;
		} else {
			$arr = explode("-", $date);
			$dd = $arr[0];
			...
			if(checkdate($mm, $dd, $yyyy)) {
				...
				$date = strtotime($date);
				if($fdate>$date) {
					$this->form_validation->set_message("todate_valid", "%s debe ser mayor que desde la fecha");
					return FALSE;
				} else {
					return TRUE;
				}
