...
class Routine extends Admin_Controller {
	...
	public function unique_teacher() {
		...
		if((int)$id) {
			...
			if(count($routine)) {
				$this->form_validation->set_message("unique_teacher", "%s ya asignó otra programación en este momento.");
				return FALSE;
			}
			return TRUE;
	...
	public function unique_section() {
		...
		if((int)$id) {
			...
			if(count($routine)) {
				$this->form_validation->set_message("unique_section", "%s ya existe");
				return FALSE;
			}
			return TRUE;
		} else {
	...
	public function unique_day() {
		$day = $this->input->post('day');
		if($day == '0') {
			$this->form_validation->set_message("unique_day", "El campo %s es obligatorio.");
			return FALSE;
		}
		return TRUE;
	}
