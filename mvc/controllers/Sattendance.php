...
class Sattendance extends Admin_Controller {
	...
	public function unique_data($data) {
		if($data != '') {
			if($data == '0') {
				$this->form_validation->set_message('unique_data', 'El campo %s es obligatorio.');
				return FALSE;
			}
			return TRUE;
		}
		...
		if($_POST) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($updateStatus) {
					...
					$retArray['message'] = "No se han encontrado datos de asistencia";
					$retArray['status'] = FALSE;
					echo json_encode($retArray);
					exit;
				}
			...
			$retArray['message'] = "El método post no encuentra";
			$retArray['status'] = FALSE;
			echo json_encode($retArray);
			exit;
		}
		...
		if($getway == "clickatell") {
			...
			if($to) {
				if($this->bulk->send($to, $message) == TRUE)  {
					...
					$result['check'] = FALSE;
					$result['message'] = "Revisa tu cuenta bulk";
					return $result;
				}
			}
		} elseif($getway == 'msg91') {
			if($to) {
				if($this->msg91->send($to, $message) == TRUE)  {
					...
					$result['check'] = FALSE;
					$result['message'] = "Revisa tu cuenta msg91";
					return $result;
				}
			}
		}
	...
	public function check_classes() {
		if($this->input->post('classesID') == 0) {
			$this->form_validation->set_message("check_classes", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
	...
	public function check_section() {
		if($this->input->post('sectionID') == 0) {
			$this->form_validation->set_message("check_section", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
	...
	public function check_subject() {
		if($this->input->post('subjectID') == 0) {
			$this->form_validation->set_message("check_subject", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
	...
	public function valid_future_date($date) {
		...
		if($date > $presentdate) {
			$this->form_validation->set_message('valid_future_date','El campo %s no tiene una fecha futura.');
			return FALSE;
		}
		return TRUE;
	}
	...
	public function check_holiday($date) {
		...
		if(count($getHolidaysArray)) {
			if(in_array($date, $getHolidaysArray)) {
				$this->form_validation->set_message('check_holiday','El campo %s dado vacaciones.');
				return FALSE;
			} else {
				return TRUE;
			}
	...
	public function check_weekendday($date) {
		...
		if(count($getWeekendDays)) {
			if(in_array($date, $getWeekendDays)) {
				$this->form_validation->set_message('check_weekendday','El campo %s dado el fin de semana.');
				return FALSE;
			} else {
				return TRUE;
			}
	...
	public function check_session_year_date() {
		...
		if($date < $startingdate || $date > $endingdate) {
			$this->form_validation->set_message('check_session_year_date','El campo %s dado no tiene salida.');
			return FALSE;
		} 
		return TRUE;
	}
