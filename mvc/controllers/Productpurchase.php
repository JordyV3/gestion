...
class Productpurchase extends Admin_Controller {
    ...
    public function unique_productsupplierID() {
        if($this->input->post('productsupplierID') == 0) {
            $this->form_validation->set_message("unique_productsupplierID", "El campo %s es obligatorio");
            return FALSE;
        }
        return TRUE;
    }
    ...
    public function unique_productwarehouseID() {
        if($this->input->post('productwarehouseID') == 0) {
            $this->form_validation->set_message("unique_productwarehouseID", "El campo %s es obligatorio");
            return FALSE;
        }
        return TRUE;
    }
                                ...
                                echo '<td data-title="'.$this->lang->line('action').'">';
                                    if($productpurchase->productpurchaserefund == 0) {
                                        if(permissionChecker('productpurchase_delete')) {
                                            echo '<a href="'.base_url('productpurchase/deletepurchasepaid/'.$productpurchasepaid->productpurchasepaidID).'" onclick="return confirm('."'".'Estás a punto de borrar un registro Esto no se puede deshacer. ¿Estás seguro?'."'".')" class="btn btn-danger btn-xs mrg" data-placement="top" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash-o"></i></a>';
                                        }
                                    }
                                echo '</td>';
                            }
