...
class Online_exam extends Admin_Controller {
    ...
    public function unique_date() {
        ...
        if($startdate != '' && $enddate != '') {
            if(strtotime($startdate) > strtotime($enddate)) {
                $this->form_validation->set_message("unique_date", "La fecha de inicio no puede ser superior a la fecha de finalización.");
                return FALSE;
            }
        }
        return TRUE;
    ...
    public function unique_date_time() {
        ...
        if($startdatetime != '' && $enddatetime != '') {
            if(strtotime($startdatetime) > strtotime($enddatetime)) {
                $this->form_validation->set_message("unique_date_time", "La fecha de inicio no puede ser superior a la fecha de finalización.");
                return FALSE;
            }
        }
        return TRUE;
    ...
    public function unique_type() {
        if($this->input->post('type') == 0) {
            $this->form_validation->set_message("unique_type", "El campo %s es obligatorio");
            return FALSE;
        }
        return TRUE;
    }
    ...
    public function unique_markType() {
        if($this->input->post('markType') == 0) {
            $this->form_validation->set_message("unique_markType", "El campo %s es obligatorio");
            return FALSE;
        }
        return TRUE;
    }
    ...
    public function unique_section() {
        if($this->input->post('classes')) {
            if($this->input->post('section') == '') {
                $this->form_validation->set_message("unique_section", "El campo %s es obligatorio");
                return FALSE;
            }
            return TRUE;
        }
    ...
    public function check_exam_question() {
        ...
        if((int)$onlineexamID) {
            ...
            if((count($online_exam_questions) == 0) && ($published == 1)) {
                $this->form_validation->set_message("check_exam_question", "Por favor agregue alguna pregunta y publique este examen.");
                return FALSE;
            }
            return TRUE;
        } else {
