...
class Mark extends Admin_Controller {
					...
					if(isset($getMarkPercentage[$markPercentage])) {
						if(is_numeric($markValue)) {
							if(0 > $markValue || $markValue > $getMarkPercentage[$markPercentage]->percentage) {
								$this->form_validation->set_message('unique_inputs', 'La calificación no puede cruzar la calificación máxima');
								return FALSE;
							}
						} else {
							if(is_string($markValue) && $markValue != '') {
								$this->form_validation->set_message('unique_inputs', 'Los datos de cadena son negables');
								return FALSE;
							}
							return TRUE;
						}
	...
	public function mark_send() {
        ...
        if($_POST) {
			...
			$retArray['message'] = 'Algo anda mal';
            echo json_encode($retArray);
            exit;
	    }
	}
