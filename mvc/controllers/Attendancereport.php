		...
		if(!empty($date)) {
			if(strlen($date) == 10) {
				...
				if(checkdate($expDate[1], $expDate[0], $expDate[2])) {
					...
					$this->form_validation->set_message('valid_date', 'El %s es dd-mm-aaaa');
					return FALSE;
				}
			} else {
				$this->form_validation->set_message('valid_date', 'El %s es dd-mm-aaaa');
	...
	public function unique_date() {
		...
		if($date != '') {
			if(($date < $startingdate) || ($date > $endingdate)) {
				$this->form_validation->set_message("unique_date", "El hasta la fecha no es válido .");
			    return FALSE;
			}
			return TRUE;
		}
	...
	public function check_weekendday($date) {
		...
		if(count($getWeekendDays)) {
			if(in_array($date, $getWeekendDays)) {
				$this->form_validation->set_message('check_holiday','El campo %s dado el fin de semana.');
				return FALSE;
			} else {
				return TRUE;
			}
