...
class Leaveapplicationreport extends Admin_Controller {
    ...
    public function date_valid($date) {
		if($date) {
			if(strlen($date) < 10) {
				$this->form_validation->set_message("date_valid", "El %s no es válido dd-mm-aaaa");
		     	return FALSE;
			} else {
		   		$arr = explode("-", $date);
		        $dd = $arr[0];
	...
	public function unique_date() {
		...
		if($fromdate != '' && $todate != '') {
			if(strtotime($fromdate) > strtotime($todate)) {
				$this->form_validation->set_message("unique_date", "La fecha de inicio no puede ser superior a la actual .");
		   		return FALSE;
			}
			if((strtotime($fromdate) < strtotime($startingdate)) || (strtotime($fromdate) > strtotime($endingdate))) {
				$this->form_validation->set_message("unique_date", "La fecha de inicio no es válida .");
			    return FALSE;
			}
			if((strtotime($todate) < strtotime($startingdate)) || (strtotime($todate) > strtotime($endingdate))) {
				$this->form_validation->set_message("unique_date", "Las fechas no son válidas .");
			    return FALSE;
			}
			return TRUE;
		}
	...
	public function date_valid_new($date) {
        if($date != '') {
			...
			if(strlen($date) < 10) {
				$this->form_validation->set_message("date_valid_new", "El %s no es válido dd-mm-aaaa");
		     	return FALSE;
			} else {
		   		$arr = explode("-", $date);
		        $dd = $arr[0];
	...
	public function unique_date_new() {
		...
		if($fromdate != '' && $todate != '') {
			if(strtotime($fromdate) > strtotime($todate)) {
				$this->form_validation->set_message("unique_date_new", "La fecha de inicio no puede ser superior a la actual .");
		   		return FALSE;
			}
			if((strtotime($fromdate) < strtotime($startingdate)) || (strtotime($fromdate) > strtotime($endingdate))) {
			...
			if((strtotime($todate) < strtotime($startingdate)) || (strtotime($todate) > strtotime($endingdate))) {
				$this->form_validation->set_message("unique_date_new", "El hasta la fecha no es válido .");
			    return FALSE;
			}
			return TRUE;
		}
    ...
    public function date_required_check() {
        ...
        $todate   = $this->input->post('todate');
        if($fromdate == '' && $todate != '') {
            $this->form_validation->set_message("date_required_check", "El campo de fecha de inicio es obligatorio.");
            return FALSE;
        } elseif($fromdate != '' && $todate == '') {
            $this->form_validation->set_message("date_required_check", "El campo hasta la fecha es obligatorio.");
            return FALSE;
        } else {
            return TRUE;
        }
