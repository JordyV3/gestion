...
class assignment extends Admin_Controller {
	...
	public function fileuploadans() {
		...
		if($_FILES["file"]['name'] !="") {
			...
			$this->form_validation->set_message("fileuploadans", "El campo %s es obligatorio");
	     	return FALSE;
		}
	}
					...
					if($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) {
						...
						if(count($assignment)) {
							if(strtotime($assignment->deadlinedate) >= strtotime(date('Y-m-d'))) {
								...
								$this->session->set_flashdata('error', 'Sumisión cerrada');
								redirect(base_url("assignment/index"));
							}
						} else {
							$this->data["subview"] = "error";
	...
	public function unique_classes() {
		if($this->input->post('classesID') == 0) {
			$this->form_validation->set_message("unique_classes", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
	...
	public function unique_section() {
		...
		if(count($sections) && $sections != FALSE && $classesID) {
			...
			if($count == 0) {
				...
				$this->form_validation->set_message("unique_section", "El %s no coincide en la clase");
	     		return FALSE;
			}
		}
		return TRUE;
	...
	public function unique_subject() {
		if($this->input->post('subjectID') == 0) {
			$this->form_validation->set_message("unique_subject", "El campo %s es obligatorio");
	     	return FALSE;
		}
		return TRUE;
	}
