...
class Mailandsms extends Admin_Controller {
	...
	function __construct () {
		...
		$this->load->library("lesson",$this->data);
		...
		if($_POST) {
			...
			if($this->input->post('type') == "email") {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$result  = $this->lesson->sendMailSystem($email, $subject, $message);
					if($result) {
						$array = array(
							'usertypeID' => '0',
	...
	public function check_email_usertypeID() {
		if($this->input->post('email_usertypeID') == 'select') {
			$this->form_validation->set_message("check_email_usertypeID", "El campo %s es obligatorio");
	     	return FALSE;
		} else {
			return TRUE;
		}
	...
	public function check_sms_usertypeID() {
		if($this->input->post('sms_usertypeID') == 'select') {
			$this->form_validation->set_message("check_sms_usertypeID", "El campo %s es obligatorio");
	     	return FALSE;
		} else {
			return TRUE;
		}
	...
	public function check_getway() {
		if($this->input->post('sms_getway') == 'select') {
			$this->form_validation->set_message("check_getway", "El campo %s es obligatorio");
	     	return FALSE;
		} else {
			...
			if(in_array($getway, $arrgetway)) {
				if($getway == "clickatell") {
					if($this->clickatell->ping() == TRUE) {
						...
						$this->form_validation->set_message("check_getway", 'Configura tu cuenta de clickatell');
	     				return FALSE;
					}
					return TRUE;
					...
					if($this->bulk->ping() == TRUE) {
						...
						$this->form_validation->set_message("check_getway", 'Usuario o contraseña No validos');
	     				return FALSE;
					}
				} elseif($getway == 'msg91') {
                    return true;
