...
class Productsale extends Admin_Controller {
    ...
    public function paidfileupload() {
        ...
        if($_FILES["productsalepaidfile"]['name'] !="") {
            ...
            if(count($explode) >= 2) {
                ...
                $this->form_validation->set_message("fileupload", "Archivo inválido");
                return FALSE;
            }
        } else {
    ...
    public function unique_productitem() {
        ...
        if($status) {
            ...
            $this->form_validation->set_message("unique_productitem", "El artículo del producto es obligatorio.");
            return FALSE;
        }
    }
                        ...
                        if(isset($productAmountArray[$productitem->productID])) {
                            ...
                            if($productAmountArray[$productitem->productID] > $totalQuantity) {
                                if(isset($productobj[$productitem->productID])) {
                                    $this->form_validation->set_message("unique_productitemadjust", "El ". $productobj[$productitem->productID]->productname." está agotado.");
                                } else {
                                    $this->form_validation->set_message("unique_productitemadjust", "El producto está agotado.");
                                }
                                return FALSE;
                            }      
                        } else {
                            $this->form_validation->set_message("unique_productitemadjust", "El producto no encontrado.");
                            return FALSE;
                        }
                    } else {
                        $this->form_validation->set_message("unique_productitemadjust", "El precio unitario y la cantidad no pueden exceder los 15 caracteres de longitud.");
                        return FALSE;
                    }
                }
            }
