...
class Media extends Admin_Controller {
	...
	public function create_folder() {
		if(permissionChecker('media_add')) {
			...
			if ($this->form_validation->run() == FALSE) {
				$this->session->set_flashdata('error', '¡Ocurrió algún error!');
			} else {
			...
			$this->session->set_flashdata('error', 'Denegación de permiso');
		}
	}
			...
			if(isset($_POST['upload_file'])) {
				if (($folder_info->userID == $userID && $folder_info->usertypeID == $usertypeID) || $usertypeID == 1) {
					if(isset($_FILES['file']['name'])=="") {
						$this->session->set_flashdata('error', 'Archivo no encontrado');
			            ...
			            if(count($explode) >= 2) {
							...
							$this->data["attachment_error"] = "Archivo inválido";
							$this->session->set_flashdata('error', 'Archivo inválido format! please upload only gif|jpg|png|pdf|docx|doc|csv|txt|ppt|xls|xlsx files');
							redirect(base_url("media/view/$folderID"));
						}
					}
				} else {
					$this->session->set_flashdata('error', '¡No estás autorizado para subir archivos en esta carpeta!');
					redirect(base_url('media/view/'.$folderID),'refresh');
				}
			}
	...
	public function add() {
		if(permissionChecker('media_add')) {
			...
			$userID = $this->session->userdata('loginuserID');
			if($_FILES['file']['name']=="") {
				$this->session->set_flashdata('error', 'Por favor seleccione el archivo!');
	...
	public function media_share() {
		if ($this->input->post('share_with') == "0") {
			$this->session->set_flashdata('error', '!Por favor, seleccione compartir con alguien!');
			...
			if($id) {
				...
				if(count($mediaCategory)) {
					if(($usertypeID == $mediaCategory->usertypeID && $userID == $mediaCategory->userID) || ($usertypeID == 1)) {
						...
						$this->session->set_flashdata('error', 'Acceso definido');
						redirect(base_url("media/index"));
					}
				} else {
