				...
				if(in_array('yes', $array['permition'])) {
					...
					if(count($dbreset)) {
						if($this->db->truncate('reset')) {
							...
							$this->session->set_flashdata('reset_error', 'restablecer el acceso desactivado');
						}
					} else {
						$this->reset_m->insert_reset(array('keyID' => $reset_key, 'email' => $email));
					}
					...
					if($this->email->send()) {
		    			...
		    			$this->session->set_flashdata('reset_error', 'Correo electrónico no enviado');
		    		}
				} else {
					$this->session->set_flashdata('reset_error', 'Correo electrónico no encontrado');
				}
						...
						if ($this->form_validation->run() == FALSE) {
							...
							foreach ($tables as $table) {
								...
								if(count($dbuser)) {
									...
									$this->session->set_flashdata('reset_success', '¡Contraseña restablecida con éxito!');
