...
class Pages extends Admin_Controller {
    ...
    public function validateDate() {
        ...
        if($error > 0) {
            $this->form_validation->set_message("validateDate", "El campo %s es obligatorio.");
        } else {
            $format = "Y-m-d H:i:s";
            ...
            if(validateDate($date, $format)) { //From helper
                ...
                $this->form_validation->set_message("validateDate", "La fecha y la hora no son válidas.");
            }
        }
    ...
    public function unique_content() {
        if($this->input->post('template') == 'none') {
            if(empty($this->input->post('content'))) {
                $this->form_validation->set_message("unique_content", "El campo %s es obligatorio.");
                return FALSE;
            }
            return TRUE;
        }
    ...
    public function unique_url() {
        ...
        if((int)$id) {
            ...
            if(count($page)) {
                $this->form_validation->set_message("unique_url", "El %s ya existe.");
                return FALSE;
            }
    ...
    public function fileUpload() {
        ...
        if($focus_id && $media_gallery_type) {
            if((int)$media_gallery_type) {
                ...
                $message = 'El ID de tipo de galería de medios no coincide';
            }
        } else {
            $message = 'Enfoque y tipo de galería de medios vacíos.';
        }
    ...
    public function deleteFileInfo() {
        ...
        if((int) $id) {
            ...
            $message = 'El ID del archivo no es entero';
        }
    ...
    public function setFileToEditor() {
        ...
        if((int)$media_type && $allID && $ulclass_type) {
                ...
                foreach ($expID as $expKey => $expValue) {
                    if(!empty($expValue)) {
                        ...
                        if(isset($sessionData[$expValue])) { 
                            ...
                            $message = 'Alguna sesion sin programar';
                        }
            ...
            $message = 'Faltan algunos datos del post';
        }
