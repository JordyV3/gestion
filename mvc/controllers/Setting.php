...
Class setting extends Admin_Controller {
	...
	public function unique_attendance() {
		if($this->input->post('attendance') === "0") {
			$this->form_validation->set_message("unique_attendance", "El campo %s es obligatorio");
			 	return FALSE;
		}
		return TRUE;
	}
	...
	public function check_attendance_smsgateway() {
		...
		if($attendance_notification =='sms' && $attendance_smsgateway == '0') {
			$this->form_validation->set_message("check_attendance_smsgateway", "El campo %s es obligatorio");
			return false;
		}
		return true;
	}
	...
	public function check_attendance_notification_template() {
		...
		if($attendance_notification =='sms' && $attendance_notification_template == '0') {
			$this->form_validation->set_message("check_attendance_notification_template", "El campo %s es obligatorio");
			return false;
		...
		if(count($weekends)) {
			foreach ($weekends as $weekend) {
				if($weekend < 0 || $weekend > 6) {
					$this->form_validation->set_message('unique_weekends', 'Los datos %s no son válidos..');
					return FALSE;
				}
			}
		} 
	...
	public function unique_ex_class($data) {
		...
		if((int)$ex_class) {
			...
			$this->form_validation->set_message('unique_ex_class', 'El campo %s es obligatorio.');
			return FALSE;
		}
	}
	...
	public function unique_time_zone($data) {
		...
		if($timezone == 'none') {
			$this->form_validation->set_message('unique_time_zone', 'El campo %s es obligatorio.');
			return FALSE;
		} else {
