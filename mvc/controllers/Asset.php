...
class Asset extends Admin_Controller {
    ...
    public function attachUpload() {
        ...
        if($_FILES["attachment"]['name'] !="") {
            ...
            if(count($explode) >= 2) {
                ...
                $this->form_validation->set_message("attachUpload", "Archivo inválido");
                return FALSE;
            }
        } else {
            if(count($asset)) {
    ...
    public function unique_serial() {
        ...
        if((int)$id) {
            ...
            if(count($asset)) {
                $this->form_validation->set_message("unique_serial", "%s ya existe");
                return FALSE;
            }
            return TRUE;
        } else {
    ...
    public function unique_category() {
        if($this->input->post('asset_categoryID') == 0) {
            $this->form_validation->set_message("unique_category", "El campo %s es obligatorio.");
            return FALSE;
        }
        return TRUE;
    }
    ...
    public function unique_loaction() {
        if($this->input->post('status') != 2) {
            if($this->input->post('asset_locationID') == 0) {
                $this->form_validation->set_message("unique_loaction", "El campo %s es obligatorio");
                return FALSE;
            }
            return TRUE;
        }
    ...
    public function unique_condition() {
        if($this->input->post('asset_condition') == 0) {
            $this->form_validation->set_message('unique_condition', "El campo %s es obligatorio.");
            return FALSE;
        }   
        return TRUE;
    }
